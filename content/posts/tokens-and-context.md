...
sub: "AI는 기억하지 않는다. 매번 전부 다시 읽는다"
excerpt: "토큰은 글을 쪼갠 단위, 컨텍스트는 한 번에 볼 수 있는 책상의 크기다. 대화가 길어지면 답이 이상해지는 이유가 여기에 있다."
cat: A
level: 1
order: 3
min: 7
asof: "2026-08"
draft: false
tldr: "토큰은 글을 잘게 쪼갠 단위이고, 컨텍스트는 **한 번에 올려 둘 수 있는 책상의 크기**입니다. AI는 대화를 기억하는 게 아니라, 매 턴마다 **지금까지의 대화를 전부 다시 읽습니다.**"
forwho:
...
todo:
  - "지금 열려 있는 긴 대화를 하나 골라 **\"지금까지 정한 것만 정리해 줘\"**를 시켜 보십시오."
  - "그 정리를 새 대화 첫 줄에 붙이고 이어가 보십시오. 답이 다시 정확해지는 걸 바로 느낍니다."
  - "반복해서 쓰는 규칙이 있으면 **대화가 아니라 파일이나 프로젝트에** 옮기십시오."
---
...
- **주제가 바뀌면 새 대화로.** 가장 효과가 크고 가장 안 하는 것입니다
- **계속 쓸 규칙은 대화가 아니라 파일·프로젝트에.** 대화는 사라지고 파일은 남습니다
- **길어졌으면 요약해서 옮기십시오.** "지금까지 정한 것만 정리해 줘" → 새 대화 첫 줄에 붙이기
